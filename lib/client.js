...
`
		...
		function apply(ctx) {
			...
			/** 每请求 Token 堆叠柱:缓存读 → 未缓存输入 → 缓存写 → 输出。固定列宽 + 横向滚动,请求再多每根柱都保持可悬浮尺寸。 */
			function TokenBars(props) {
				...
				var BAR_W = 8;
				var H = 170;
				var W = Math.max(800, points.length * SLOT);
				var maxTotal = 1;
				...
				for (var j = 0; j < points.length; j++) {
					...
					for (var r = 0; r < rows.length; r++) {
						...
						segs.push(react.createElement("rect", {
							key: "s" + r,
							x: x, y: H - acc - sh, width: BAR_W, height: sh,
							fill: rows[r].c,
						}));
						acc += sh;
					}
					...
					if (ov > 0) {
						...
						segs.push(react.createElement("rect", {
							key: "out",
							x: x, y: H - acc - oh, width: BAR_W, height: oh,
							fill: COLORS.output,
						}));
						acc += oh;
					}
					...
					rects.push(react.createElement("g", {
						...
						react.createElement("rect", {
							x: x, y: hoverIdx === j ? y - 1 : y, width: BAR_W, height: hoverIdx === j ? h + 1 : h,
							fill: "transparent",
						}),
						segs,
				...
				return react.createElement("div", { className: "utx-panel" },
					...
					react.createElement("div", { className: "utx-chart-scroll" },
						react.createElement("svg", { className: "utx-chart-svg-fixed", width: W, height: H, viewBox: "0 0 " + W + " " + H },
							gridLines(W, H, 3),
							rects,
						),
					),
					react.createElement("div", { className: "utx-detail" }, detailText(points, hoverIdx, "bars")),
					react.createElement("div", { className: "utx-note" }, "纵轴峰值 " + fmt(maxTotal) + " tokens · 共 " + points.length + " 次请求" + (points.length * SLOT > 800 ? " · 固定列宽,可横向滚动" : "")),
				);
			}

			/** 累计计费输入/输出曲线。 */
			function CumulativeLines(props) {
				var points = props.points;
				var W = 800;
				var H = 170;
				var cumIn = [];
			...
			/** 每请求墙钟耗时柱。固定列宽 + 横向滚动,与 Token 柱共用同一列宽规格。 */
			function DurationBars(props) {
				var points = props.points;
				var hover = react.useState(null);
				var hoverIdx = hover[0];
				var setHoverIdx = hover[1];
				...
				return react.createElement("div", { className: "utx-panel" },
					...
					react.createElement("div", { className: "utx-note" }, "峰值 " + fmtDur(maxDur) + (points.length * SLOT > 800 ? " · 固定列宽,可横向滚动" : "")),
				);
			}
			...
			function TrendsView(props) {
				...
				if (status === "error") {
					...
					body = react.createElement("div", { className: "utx-body" },
						react.createElement(StatsCards, { points: points }),
						react.createElement(TokenBars, { points: points }),
						react.createElement(CumulativeLines, { points: points }),
						react.createElement(DurationBars, { points: points }),
